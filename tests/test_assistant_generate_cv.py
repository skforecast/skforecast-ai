# Unit test generate_cv ForecastingAssistant
...
def test_generate_cv_ValueError_when_initial_train_size_float_out_of_range(value):
    """
    Test that generate_cv() raises ValueError when initial_train_size is a
    ...
    with pytest.raises(ValueError, match=err_msg):
        assistant.generate_cv(profile, plan, initial_train_size=value)


def test_generate_cv_ValueError_when_fewer_than_2_folds():
    """
    Test that generate_cv() raises ValueError when the configuration
    ...
    with pytest.raises(ValueError, match=err_msg):
        assistant.generate_cv(profile, plan, initial_train_size=20)


# =============================================================================
# Tests: basic output
# =============================================================================
def test_generate_cv_output_when_single_series_defaults():
    """
    Test that generate_cv() returns a tuple of (TimeSeriesFold, str) with
    ...
    result = assistant.generate_cv(profile, plan)
...
def test_generate_cv_output_when_default_initial_train_size():
    """
...
def test_generate_cv_output_when_steps_from_plan():
    """
    Test that cv.steps equals plan.steps.
    """
    assistant = ForecastingAssistant()
    profile = assistant.profile(data=df_single, target="sales", date_column="date")
    plan = assistant.generate_plan(profile, steps=7)
...
def test_generate_cv_output_when_multi_series_defaults():
    """
    Test that generate_cv() works for multi-series profiles.
    """
    assistant = ForecastingAssistant()
...
# =============================================================================
def test_generate_cv_output_when_initial_train_size_int_override():
    """
    ...
    cv, _ = assistant.generate_cv(profile, plan, initial_train_size=50)

    assert cv.initial_train_size == 50


def test_generate_cv_output_when_initial_train_size_float_override():
    """
    ...
    cv, _ = assistant.generate_cv(profile, plan, initial_train_size=0.5)

    # 50% of 100 observations = 50
    assert cv.initial_train_size == 50


def test_generate_cv_output_when_initial_train_size_str_skips_validation():
    """
    ...
    cv, explanation = assistant.generate_cv(
...
def test_generate_cv_output_when_refit_override():
    """
    ...
    cv, _ = assistant.generate_cv(profile, plan, refit=3)

    assert cv.refit == 3


def test_generate_cv_output_when_fixed_train_size_override():
    """
    ...
    cv, _ = assistant.generate_cv(profile, plan, fixed_train_size=True)

    assert cv.fixed_train_size is True


def test_generate_cv_output_when_gap_override():
    """
    ...
    cv, _ = assistant.generate_cv(profile, plan, gap=2)
...
# =============================================================================
def test_generate_cv_output_when_floor_by_lags_int():
    """
    ...
    plan = assistant.generate_plan(profile, steps=1)
...
def test_generate_cv_output_when_floor_by_lags_list():
    """
...
def test_generate_cv_output_when_statistical_floor():
    """
...
def test_generate_cv_output_when_differentiation_set():
    """
...
# Tests: explanation
# =============================================================================
def test_generate_cv_explanation_contains_key_params():
    """
    ...
    _, explanation = assistant.generate_cv(profile, plan)
...
# Tests: LLM path
# =============================================================================
def test_generate_cv_LLMRequiredError_when_prompt_but_no_llm():
    """
    Test that generate_cv() raises LLMRequiredError when prompt is
    ...
    err_msg = re.escape(
        "`generate_cv()` requires an LLM. "
        "Pass `llm=...` when creating ForecastingAssistant."
    )
    with pytest.raises(LLMRequiredError, match=err_msg):
        assistant.generate_cv(profile, plan, prompt="I retrain weekly")


def test_generate_cv_llm_success(monkeypatch):
    """
    Test that generate_cv() uses LLM-returned parameters when prompt is
...
def test_generate_cv_llm_kwargs_override_llm(monkeypatch):
    """
...
def test_generate_cv_llm_retry_then_success(monkeypatch):
    """
    Test that generate_cv() retries on validation failure and succeeds
    ...
    cv, _ = assistant.generate_cv(profile, plan, prompt="Forecast ahead")

    assert cv.initial_train_size == 50
    assert call_count["n"] == 2


def test_generate_cv_llm_all_retries_fail_deterministic_fallback(monkeypatch):
    """
    Test that generate_cv() falls back to deterministic defaults after
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        cv, _ = assistant.generate_cv(profile, plan, prompt="Bad scenario")
...
def test_generate_cv_llm_explanation_includes_reasoning(monkeypatch):
    """
    ...
    _, explanation = assistant.generate_cv(
...
def test_generate_cv_deterministic_when_no_prompt_and_llm_configured():
    """
    Test that generate_cv() uses deterministic defaults when no prompt is
