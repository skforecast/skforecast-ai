...
def test_build_comparison_context_output_when_all_candidates_succeed():
    ...
    assert "<comparison_overview>" in context
    ...
    assert "Name: winner" in context

    assert "failed_candidates" not in context
...
def test_build_comparison_context_output_when_some_candidates_fail():
    ...
    assert "<failed_candidates>" in context
...
# =============================================================================
# Tests: tagged context block
# =============================================================================
def test_build_comparison_context_wraps_sections_in_single_block():
    """
    Test that the comparison renders exactly one <forecast_context> block
    with balanced tags, including the winning candidate's nested plan.
    """
    comparison = make_comparison_result(assistant, with_failure=True)

    context = build_comparison_context(comparison)

    assert context.startswith("<forecast_context>")
    assert context.endswith("</forecast_context>")
...
def test_build_comparison_context_no_markdown_headings():
    """
    Test that the comparison context emits no markdown headings, which
    would collide with the skills and with the expected answer format.
    """
    comparison = make_comparison_result(assistant)
