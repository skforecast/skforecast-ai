...
def test_run_forecast_single_series_returns_predictions():
    """
    Test that run_forecast returns predictions with length equal to horizon
    ...
    assert len(result["predictions"]) == plan_single.horizon
...
def test_run_forecast_single_series_with_intervals():
    ...
    assert len(result["intervals"]) == plan_single_with_intervals.horizon
...
def test_run_forecast_multi_series_returns_predictions():
    ...
    assert isinstance(result["predictions"], pd.DataFrame)
    # Multi-series returns horizon * n_series rows
    n_series = profile_multi.n_series
    assert len(result["predictions"]) == plan_multi.horizon * n_series
    assert result["metric_name"] == "mean_absolute_error"
    assert isinstance(result["metric_value"], float)
    assert result["metric_value"] > 0
...
def test_run_forecast_statistical_returns_predictions():
    ...
    assert isinstance(result["predictions"], pd.DataFrame)
    assert len(result["predictions"]) == plan_stats.horizon
    assert isinstance(result["metric_value"], float)
    assert result["intervals"] is not None
...
def test_validate_run_inputs_warns_horizon_exceeds_test_size():
    """
    Test that validate_run_inputs warns when horizon exceeds the test set size.
    """
    plan_large_horizon = ForecastPlan(
        ...
        metric="mean_absolute_error",
        backtesting_strategy="TimeSeriesFold",
        rationale="Large horizon.",
    )

    warnings = validate_run_inputs(
        data=df_single, profile=profile_single, plan=plan_large_horizon
    )
