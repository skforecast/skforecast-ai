...
**Deterministic mode (the default).** Create the assistant with no arguments and it runs the full pipeline, from profiling to execution, without an internet connection or an API key. Every method runs in this mode; only `ask()` and `create_cv()` with a `prompt` require a configured model.
...
The risk with any AI explainer is **drift**: the engine applies one rule, but the model explains a different one from outdated training data, and trust erodes.
