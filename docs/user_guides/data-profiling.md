...
The profiler first resolves the data format and then extracts metadata sequentially:
...
flowchart LR
    ...
    A(Format Resolution):::step --> B(Resolve Dates):::step
    B --> C(Infer Frequency):::step
    C --> D(Dimensionality & Quality Scans):::step
    D --> E(Train/Test Boundaries):::step
    E --> F(Seasonality):::step
    F --> G{DataProfile}:::output
```
...
3. **Infers Frequency:** Analyzes the time deltas between rows (`pd.infer_freq`) to infer the Pandas frequency string (e.g., `'D'` for daily, `'MS'` for month-start). It also checks if the frequency is already explicitly set on the index (`frequency_is_set`).
...
6. **Detects Gaps:** Uses the inferred frequency to check if there are missing timestamps between the start and end of the date range.

### Step 3: Dimensionality and Quality Checks
The profiler scans the actual data columns:
1. **Target Identification & Statistics:** Locates the target variable(s) and determines their data type (numeric, categorical). It then computes robust descriptive statistics (`target_stats`): min, max, mean, and standard deviation for each target series.
...
### Step 4: Train/Test Boundary Calculations
To assist with downstream validation and backtesting scripts, the profiler pre-calculates essential splitting references:
1. **`end_train`:** It calculates the exact datetime representing the 80% mark of the dataset. This provides a deterministic default for train/test splits.
2. **`start_date`:** It establishes the reference start date for position-to-date conversions. For long-format data where series might start at different times, it carefully identifies the *latest* starting date across all series to ensure safe, aligned slicing.

### Step 5: Seasonality Heuristics
